...
public class DisciplinaJpaController implements Serializable {
    ...
    public List<Disciplina> listarDisciplinaSemCurso() {
        ...
        em.getTransaction().begin();
        try {
            Query query = em.createQuery("SELECT e FROM Disciplina e WHERE e.curso is null");
            List<Disciplina> disciplina = query.getResultList();
            return disciplina;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Disciplina> listaDisciplinaProfessor(int iProf){
        ...
        em.getTransaction().begin();
        try {
            Query query = em.createQuery("SELECT e FROM Disciplina e join e.professor prof WHERE prof.id = :prof"); // Condição proibido
            query.setParameter("prof", iProf);
            List<Disciplina> res = query.getResultList();
            return res;
        } catch (Exception e) {
            e.printStackTrace();
            em.getTransaction().rollback();
